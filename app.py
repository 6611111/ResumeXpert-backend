...
# Example job profiles for ML-based recommendation
JOB_PROFILES = {
...
def nlp_section_scores(text):
    ...
    for section, keywords in SECTION_KEYWORDS.items():
        ...
        section_scores[section] = min(score * 20, 100)  # scale to max 100
    return section_scores

def nlp_total_score(section_scores):
    total = int(np.mean(list(section_scores.values())))
    return total
...
def recommend_job_roles(text, top_n=3):
    ...
    recommendations = [
        {"role": all_roles[i], "score": round(scores[i]*1000, 2)}
